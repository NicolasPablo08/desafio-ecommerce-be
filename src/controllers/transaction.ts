"use server";
import { Transaction, User } from "src/models/model";
import { getProductById } from "src/controllers/products";
import { createSingleProductPreference } from "src/lib/mercadopago";
import { sendConfirmedPaymentToEmail } from "src/lib/sendGrid";
export async function createOrder(productId: string, userId: string) {
  try {
    const searchProduct = await getProductById(productId);
    if (!searchProduct)
      throw new Error("Error: product not found from createOrder of transaction controller");
...
export async function getOrderById(orderId: string, userId: string) {
  try {
    const order = await Transaction.findTransactionByOrderIdAndUserId(orderId, userId);
    return order;
  } catch (e) {
    throw new Error(
...
export async function confirmPurchase(purchaseId: string) {
  ...
  try {
    const confirmTransaction = await Transaction.updateStatus(purchaseId);
    if (!confirmTransaction)
      throw new Error(
        "Error: could not confirm transaction from confirmPurchase of transaction controller"
      );
    const transaction = (await Transaction.findTransactionByOrderId(purchaseId)) as Transaction;
    if (!transaction)
      throw new Error(
        "Error: could not search transaction from confirmPurchase of transaction controller"
      );
    const user = await User.findUserById(transaction.userId);
    if (!user)
      throw new Error(
    ...
    const sendEmailWithConfirmedPayment = sendConfirmedPaymentToEmail(user.email, purchaseId);
