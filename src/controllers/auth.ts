...
import { sendCodeToEmail } from "src/lib/sendGrid";
...
export async function findOrCreateAuth(email: string) {
  try {
    ...
    if (!auth) {
      ...
      if (!newUser) throw new Error("Error creating user from findOrCreateAuth of auth controller");
    ...
    if (!sendCode) throw new Error("Error sending email from findOrCreateAuth of auth controller");

    return { message: "Un código ha sido enviado a tu email." };
  } catch (e) {
    throw new Error(`Error in findOrCreateAuth from controller Auth: ${e.message}`);
  }
}
...
export async function getToken(email: string, code: number) {
  try {
    ...
    if (!auth) throw new Error("Error: Auth not found from getToken of auth controller");
    // 2) Validar código
    if (code !== auth.code) throw new Error("Error: invalid code from getToken of auth controller");
    const now = new Date();
    ...
    if (!token) throw new Error("Error generating token from getToken of auth controller");
